...
func connection() (*sql.DB, error) {
	db, err := sql.Open("sqlite3", "/Users/Nastya/GoglandProjects/prog/.idea/foo.db")
...
func checkDatabase(authdata authData) (bool) {
	...
	//connect to database
	db, _ := connection()
	rows, _ := db.Query("SELECT uid FROM authData WHERE username=? and passwordHash=?", authdata.userName, authdata.userPassword)
